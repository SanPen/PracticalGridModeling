...
\marginnote{The series admittance $Y_{serie}$ is computed as the inverse of $Z_{ABC}$. From the line's calculation it is obtained the series impedance matrix $Z_{ABC}$ and the shunt admittance matrix $Y_{sh}$.}
...
The transformer model implements the branch model as well, but the model admittances $Y_{pp}$, $Y_{ps}$, $Y_{sp}$ and $Y_{ss}$ vary depending on the connections of the transformer at the primary and at the secondary.
...
$y_t$ is the transformer winding per unit admittance. It is given in values per units from the transformer especifications sheet. Usually it is given either directly as leakeage resistance $r_l$ and inductance $x_l$, in which case:
...
Since all the electrical models are in per-unit values, the voltage regulator is modeled exactly the same as a transformer.
