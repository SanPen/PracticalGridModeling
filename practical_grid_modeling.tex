...
\marginnote{Note that the equation \ref{eq:power_flow} is nonlinear when solving for the voltage ($V$). However if there are no power injections and only current injections, the equation becomes linear. Because of the non-linearity, we will be using iterative methods to solve this equation.}
...
We repeat equations \ref{eq:zm_ik} to \ref{eq:zm_copy_sol} untill the error is less or equal to a given tolerance not too strict like $1\times10^{-3}$
...
%int max_iter = 20;
%int iter;
%arma::cx_vec Ivd, Ik, Vk, Ck, Vprev;
%
%// slack-injected currents
%Ivd = Yslack * Vslack;
%
%// slack voltages influence
%Ck =  arma::spsolve(Yred, Ivd, "lapack");
%
%// copy the voltage
%Vprev = Vred;
%
%// compute the currents
%Ik = arma::conj(Sred / Vprev) + Ired;
%
%// compute the voltages
%Vk = arma::spsolve(Yred, Ik, "lapack") - Ck;
%
%// compute the tolerance
%tol = arma::norm(Vprev - Vk, "inf");
%iter = 1;
%max_tol = 1e-3;
%
...
Ray D. Zimmerman came up with a fantastic way of building the Jacobian matrix \cite{zimmerman2010ac} by using lineal algebra operations over the complex matrices and vectors. The formulation presents a huge advantage: No use of cosine and sine operations for the derivatives computation. The calculation of trigonometric functions is a recursive operation and should be avoided whenever possible in high performance computing. The formulation is presented below.

First we compute the total current injection. It is a sparse diagonal matrix, so do not use a dense matrix format.
\begin{equation}
I_{diag} = diag(Y \times V - I_{bus})
\end{equation}
...
Finally, assemble the Jacobian matrix $J$. The Jacobian matrix is sparse and only contains real values. If the circuit contains $npq$ buses of type $PQ$ and $npv$ buses of type $PV$, the Jacobian matrix is a square matrix with $2 npq + npv$ rows and the same number of columns.
...
Newton-Raphson is a iterative numerical method that minimizes the value of a function. In our case the value to minimize is the power mismatch:
...
\marginnote{Here we use the auxiliary vectors $pqpv$ and $pq$ wich contain the indices of the pq and pv type buses, and the pq type buses respectivelly. $npqpv$ and $npq$ are the sizes of those two vectors.}
\begin{equation}
...
%    dx = arma::spsolve(A, rhs, "lapack");
%
%    // objective function to minimize
%    f = 0.5 * arma::dot(dz, dz);
%
%    // decision function
%    rho = (f_prev - f) / (0.5 * arma::dot(dx, lbmda * dx + rhs));

Next, we solve the voltage increments vector $\Delta x$. As in Newton-Raphson, this vector has $2npq+npv$ elements and the structure is the same as the one explained in the Newton-Raphson method.
...
Then, compute the objective function to minimize $f$. It is a value.
...
Then the decision function value $\rho$. It is a value.
...
%        lbmda *= std::max(1.0 / 3.0, 1 - std::pow(2 * rho - 1, 3.0));
%        nu = 2.0;
...
%double nu = 2.0;
%double lbmda = 0.0;
%double f_prev = 1e9;  // very large number
%double f;
%double rho;
%unsigned long npq = calc->pq.size();
%unsigned long npqpv = calc->pqpv.size();
%
%
%arma::sp_mat Idn; // identity matrix
%Idn.eye(2 * npqpv, 2 * npqpv);
%
%arma::sp_mat H, H1, H2, A;
%arma::vec dz, dx, rhs;
%arma::vec dVm(calc->n_buses);
%arma::vec dVa(calc->n_buses);
%arma::cx_vec Vsol = calc->Vbus[p];
%arma::vec Vm = arma::abs(Vsol);
%arma::vec Va = arma::arg(Vsol);
%
%// fill zeros only where needed
%for(unsigned int i:calc->vdpv)
%    dVm(i) = 0.0;
%for(unsigned int i:calc->vd)
%    dVa(i) = 0.0;
%
%// fill the Jacobian and the mismatch
%calcJ(&H, &dz, &(calc->Ybus[p]), &Vsol, &(calc->Ibus[p]), &(calc->Sbus[p]));
%
%err = arma::norm(dz, "inf");
%converged = err < tol;
%
...
%    A = H2 + lbmda * Idn;
%
%    // right hand side: H^t·dz
%    rhs = H1 * dz;
%
%    // solve the liear system: is negative so, it is summed to the voltage solution
%    dx = arma::spsolve(A, rhs, "lapack");
%
%    // objective function to minimize
%    f = 0.5 * arma::dot(dz, dz);
...
%        nu = 2.0;
%
%        // convert dx to voltage increments
%        solution_to_va(&dVm, &dVa, &dx, npqpv, npq);
%
%        // modify the voltage solution properly
%        Vm += dVm;
%        Va += dVa;
%        Vsol = Vm % (arma::cos(Va) + arma::cx_double(0.0, 1.0) * arma::sin(Va));
%        // update Vm and Va again in case we wrapped around with a negative Vm (numerical trick!)
%        Vm = arma::abs(Vsol);
%        Va = arma::arg(Vsol);
%
%    } else {
