...
%	\color{maincolor}
	\begin{tabular}{|l|l|l|l|l|}
		...
		Bus 0   & False     & 230.0 & 0.9  & 1.1  \\ \hline
		Bus 1   & False     & 230.0 & 0.9  & 1.1  \\ \hline
		Bus 2   & False     & 230.0 & 0.9  & 1.1  \\ \hline
		Bus 3   & True      & 230.0 & 0.9  & 1.1  \\ \hline
		Bus 4   & False     & 230.0 & 0.9  & 1.1  \\ \hline
	\end{tabular}
\caption{Bus data}
...
%	\color{maincolor}
	\begin{tabular}{|l|l|l|l|l|}
		\hline
		\rowcolor{maincolor}
		{\color{white}name} & {\color{white}bus}    & {\color{white}Z}  & {\color{white}I}  & {\color{white}S}         \\ \hline
		Load 1 & Bus 1 &  0j & 0j & 300 + 98.61j   \\ \hline
		Load 2 & Bus 2 &  0j & 0j & 300 + 98.61j   \\ \hline
		Load 3 & Bus 3 &  0j & 0j & 99.99 + 99.99j \\ \hline
	\end{tabular}
\caption{Load data}
...
2.1+0j,  & -3-0.9861j,  & 0.2349-0.9861j, &  -0.9999-0.9999j,  & 4.6651+0j \\
\end{array} \right] p.u.
$$
...
22.25-222.48j & -3.525+35.23j & 0j            & -3.26+32.57j & -15.47+154.70j\\
-3.52+35.23j  & 12.69-126.9j  & -9.17+91.68j  & 0j           & 0j \\ 
0j            & -9.17+91.68j  & 12.50-125.0j  & -3.33+33.34j & 0j \\
-3.26+32.57j  & 0j            & -3.33+33.34j  & 9.92-99.23j  & -3.33+33.34j \\
-15.47+154.70j& 0j            & 0j            & -3.34+33.34j & 18.80-188.02j
\end{array} \right] p.u.
$$
...
\subsection{Shunt connected impedances/admittances}
...
\begin{marginfigure}
	...
	\caption{Delta/Wye shunt model.}
	\label{delta_shunt_model}
\end{marginfigure}


\section{Voltage controlled device}
...
\begin{itemize}
	\item $S$: Nodal power injections vector in p.u.
	\item $V$: Nodal voltages vector.
	\item $I$:  Nodal current injections vector.
	\item $Y_{zip}$:  Nodal shunt admittances vector. These are the admittances corresponding the ZIP model (YISV model in this document). These admittances are added to the diagonal of the admittance matrix ($Y_{bus}$).
\end{itemize}
...
\begin{equation}
{\textbf{I}_f = (\textbf{C}_f \times \textbf{Y}_{bus}) \times \textbf{V}}
\end{equation}

\begin{equation}
{\textbf{I}_t = (\textbf{C}_t \times \textbf{Y}_{bus}) \times \textbf{V}}
\end{equation}
...
\begin{equation}
{\textbf{S}_f = (\textbf{C}_f \times \textbf{V}) \cdot \textbf{I}_f^*}
\end{equation}

\begin{equation}
{\textbf{S}_t = (\textbf{C}_t \times \textbf{V}) \cdot \textbf{I}_t^*}
\end{equation}
...
\begin{itemize}
	\item $\textbf{Y}_{bus}$: Bus admittance matrix in p.u. See \ref{Y_calculation}.
	\item $\textbf{C}_f$: Connectivity matrix of the branches and the \textit{from} buses.
	\item $\textbf{C}_t$:Connectivity matrix of the branches and the \textit{to} buses.
	\item $\textbf{V}$: Array of bus voltages in p.u.
...
\begin{enumerate}
	...
	\begin{itemize}
		\item If the bus voltage magnitude $V_i \geq Vset_i$, then it remains a $PQ$ bus at current iteration and set $Q_i = Qmin_i$.
		
		\item If $V_i < Vset_i$ , then compare $Q_i$ with the upper and lower limits.
		...
		\begin{itemize}
			...
			\item If $Qmin_i < Q_i < Qmax_i$ , then it is switched to $PV$ bus, set $V_i = Vset_i$.
		\end{itemize}
	\end{itemize}
	...
	\begin{itemize}
		\item If its voltage magnitude $V_i \leq Vset_i$ , then the bus $i$ still a PQ bus and set $Q_i = Qmax_i$.
	
		\item If $V_i > Vset_i$ , then compare between $Q_i$ and its upper/lower limits:
		...
		\begin{itemize}
			...
			\item If $Qmin_i < Q_i < Qmax_i$ , then it is switched to $PV$ bus and set $V_i = Vset_i$
		\end{itemize}
	\end{itemize}
...
\begin{equation}
I_f = C_f \times Y_{bus} \times V = Y_f \times V 
